...
bw$year <- factor(bw$year, levels = c(2002, 2007, 2012, 2017))
colors=c("#44AA99", "#88CCEE", "#DDCC77", "#CC6677")
names(colors) <- levels(bw$year)

bw$alpha <- as.factor(ifelse(bw$category == "ave", 0.4, 1))

# put  median and average acres per operation next to each other in box and whisker!
dist_acres <- ggplot(bw, aes(x=year, y=acres, fill=year, alpha = factor(alpha))) +
  geom_boxplot() +
  project_theme + 
...
ggsave("./viz/Figure7.png", dpi = 400, width = 200, height = 80, units = "mm")
```

...
Average Farm HH income and US HH income in Current $. Data: US BEA.
...
ggsave("./viz/Figure5.png", dpi = 400, width = 150, height = 90, unit = "mm")
```
...
```{r}
c <- readRDS("./data/contemp-coa.RDS")
nc <- c %>% filter(GEOID %in% counties$GEOID) %>% filter(year == 2017 | year == 2002) %>% filter(!is.na(net_income))

# STATS
c %>% filter(year == 2017, net_income < 0) %>% arrange(net_income) # 444/3034 reporting counties
c %>% filter(year == 2002, net_income < 0) %>% arrange(net_income) #416/3034
#464 counties > 10%
